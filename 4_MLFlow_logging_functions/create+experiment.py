...
if __name__ == "__main__":
    ...
    # # create experiment with artifact location (returns experiment id)
    # exp_id = mlflow.create_experiment(
    #     name="exp_create_exp_artifact",
    #     tags={"version": "v1", "priority": "p1"}, # additional metadata for your experiment (easily identify experiments and filter them later)
    #     artifact_location=Path.cwd().joinpath("myartifacts").as_uri() # create new folder myartifacts in current working directory (make it compatible with MLflow by converting to URI format)
    # )

    # # take experiment ID and return all stored information related to experiment
    ...
    with mlflow.start_run(experiment_id=exp.experiment_id): # specify experiment id to log run under that experiment
